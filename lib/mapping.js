...
export function deriveParams(name, options) {
  ...
  const { melody, weights, melodyCycles } = bytesToMelody(bytes, {
    ...
    nibbleMap: Array.isArray(options.nibbleMap) && options.nibbleMap.length === 16 ? options.nibbleMap : null,
  });

  return { scale, cpm, melody, bytes, seed, events: melodyCycles || 1, byteWeights: weights };
}

function noteFromNibble(n, map) {
  if (map) return map[n & 0xf];
  return n - 8;
}

const TAIL_RESTS = 12;
const TAIL_REST_WEIGHT = 1;
...
function bytesToMelody(bytes, sub) {
  ...
  if (bytes.length === 0) {
    ...
    for (let i = 0; i < bytes.length; i++) {
      ...
      nibbleNotes.push(noteFromNibble(hiV, sub.nibbleMap) + offset + hiAdj);
      nibbleNotes.push(noteFromNibble(loV, sub.nibbleMap) + offset + loAdj);
    }
