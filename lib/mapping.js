...
function byteChord(byte, offset, hiAdj, loAdj) {
  const hi = noteFromNibble((byte >> 4) & 0xf) + offset + hiAdj;
  const lo = noteFromNibble(byte & 0xf) + offset + loAdj;
  return `[${hi},${lo}]`;
}
...
function bytesToMelody(bytes, sub) {
  ...
  if (bytes.length === 0) {
    ...
    for (let i = 0; i < bytes.length; i++) {
      const chord = byteChord(bytes[i], offset, hiAdj, loAdj);
      const shouldSub =
      ...
      if (shouldSub) {
        ...
        for (let k = 1; k < size; k++) {
          ...
          subTokens.push(byteChord(sibling, offset, hiAdj, loAdj));
        }
