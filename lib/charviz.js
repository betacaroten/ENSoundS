...
export function animateCharViz(spans, noteSeconds, totalEvents, loop = false) {
  ...
  const step = (now) => {
    const elapsed = (now - start) / noteMs;
    if (!loop && elapsed >= totalEvents) {
      clearCharViz(spans);
      rafId = 0;
      return;
    }
    const cyc = loop ? ((elapsed % totalEvents) + totalEvents) % totalEvents : elapsed;
    for (const s of spans) {
      const active = s.start >= 0 && cyc >= s.start && cyc < s.end;
