...
export function autoTweakOptions(name, options) {
  ...
  return {
    ...options,
    leadAdsr: [
      round2(clamp(adsr[0] + lerp(norm(b1), -0.05, 0.05), 0, 2)),
      adsr[1],
      adsr[2],
      round2(clamp(adsr[3] + lerp(norm(b2), -0.15, 0.15), 0, 3)),
    ],
    leadLpf: Math.round(clamp(baseLpf * lerp(norm(b3), 0.9, 1.1), 100, 20000)),
