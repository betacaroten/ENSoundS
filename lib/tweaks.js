...
export function autoTweakOptions(name, options) {
  ...
  return {
    ...options,
    leadSynth: SYNTHS[b0 % SYNTHS.length],
    leadAdsr: [
      round2(clamp(adsr[0] + lerp(norm(b1), -0.1, 0.1), 0, 2)),
      adsr[1],
      adsr[2],
      round2(clamp(adsr[3] + lerp(norm(b2), -0.3, 0.3), 0, 3)),
    ],
    leadLpf: Math.round(clamp(baseLpf * lerp(norm(b3), 0.75, 1.25), 100, 20000)),
    leadVib: round2(lerp(norm(b4), 0, 1.3)),
    masterRoom: round2(clamp(baseRoom + lerp(norm(b5), -0.2, 0.2), 0, 1)),
  };
}
